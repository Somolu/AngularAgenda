...
export class ShowComponent implements OnInit {
  ...
  showTask() {
    this.taskService.getTaskById(this.data.id||'0').subscribe(
      (d: ITask) => {
  ...
  updateTask() {
    this.taskService.updateTask(this.taskForm.value, this.data.id||'0').subscribe(
      (d) => {
  ...
  onDeleteTask() {
    this.taskService.deleteTask(this.data.id||'0').subscribe(
      (d) => {
        console.log(d);
        this.dialogRef.close();
      },
